...
def obtenerListaBlanca(id, monto, tipo):
    sql = "SELECT * FROM listablanca WHERE idCliente = %s and montoTransaccion = %s and idTransaccionTipo = %s;"
    datos = (id,monto,tipo)
    query.Cursor.execute(sql,datos)
    tranzacciones = query.Cursor.fetchall()
    query.conn.commit()
...
def guardarHistorial(id):
    sql = "INSERT INTO historialbloqueo VALUES (NULL,%s,CURRENT_DATE(),1)"
    datos = (id)
    query.Cursor.execute(sql,datos)
    query.conn.commit()
...
def montoMes():
    sql="SELECT MONTHNAME(fechaTransaccion) AS mes, FORMAT(SUM(montoTransaccion), 2) monto FROM detalletransaccioncuenta WHERE tipoFraude=1 GROUP BY mes ORDER BY fechaTransaccion DESC LIMIT 5"
    query.Cursor.execute(sql)
    tranzacciones = query.Cursor.fetchall()
    query.conn.commit()
    return tranzacciones
...
def getaccuount(id):
    sql="SELECT * FROM cuentabancaria WHERE idCliente = "+str(id)+";"
    query.Cursor.execute(sql)
    tranzacciones = query.Cursor.fetchall()
    query.conn.commit()
    return tranzacciones